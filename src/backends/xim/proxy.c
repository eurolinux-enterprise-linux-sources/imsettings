	...
	switch (XIM_CLIENT_CONNECTION (conn)->reconnection_state) {
	    ...
	    case RECONNECT_OP_ENCODING_NEGOTIATION:
		    ...
		    G_STMT_START {
			    ...
			    for (i = 0; i < a->len; i++) {
				    GValue *v = g_array_index(a, GValue *, i);
							      ...
							      gpointer      data)
{
	...
	if (XIM_CLIENT_CONNECTION (proto)->is_reconnecting) {
		...
		if (category == 0) {
			v = g_array_index(G_XIM_CONNECTION (conn)->encodings,
					  const GValue *, index_);
			e1 = g_xim_str_get_string(g_value_get_boxed(v));
		} else {
			v = g_array_index(G_XIM_CONNECTION (conn)->encoding_details,
					  const GValue *, index_);
						 ...
						 gpointer      data)
{
	...
	if (c->encodings)
		g_array_unref(c->encodings);
	if (c->encoding_details)
		g_array_unref(c->encoding_details);
	c->encodings = g_array_new(TRUE, TRUE, g_slist_length(encodings));
	for (l = encodings; l != NULL; l = g_slist_next(l)) {
		GValue v = { 0, };

		g_value_init(&v, G_TYPE_XIM_STR);
		g_value_set_boxed(&v, l->data);
		g_array_append_vals(c->encodings, &v, 1);
	}
	c->encoding_details = g_array_new(TRUE, TRUE, g_slist_length(details));
	for (l = details; l != NULL; l = g_slist_next(l)) {
		GValue v = { 0, };

		g_value_init(&v, G_TYPE_XIM_ENCODINGINFO);
		g_value_set_boxed(&v, l->data);
		g_array_append_vals(c->encoding_details, &v, 1);
	}
